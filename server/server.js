...
import serviceAccountKey from './reactjs-blogging-website-7fa23-firebase-adminsdk-lvyp3-46088566af.json' assert {type: 'json'}
...
// verifying access token to give publish access
...
server.post('/signup', (req, res) => {
    ...
    bcrypt.hash(password, 10, async (err, hashed_password) => {
        ...
        user.save().then((u) => res.status(200).json(formatDatatoSend(u)))
            .catch((err) => {
                if (err.code == 11000) {
                    console.log(err)
    ...
    User.findOne({ "personal_info.email": email })
        .then((user) => {
            ...
            if (!user.google_auth) {
                ...
                return res.status(403).json({ 'error': 'Account was created using Google. Login with Google' })
            }
...
server.post('/google-auth', async (req, res) => {
    ...
    getAuth()
        ...
        .then(async (decodedUser) => {
            ...
            if (user) {
                if (!user.google_auth) {
                    return res.status(403).json({ 'error': 'This email has already been signed up without Google. Please login with password to access the account' })
                }
            }
            else {
                let username = await generateUsername(email);
        ...
        .catch(err => {
            return res.status(500).json({ 'error': 'Failed to authenticate with Google.Try with some other account' })
        })
})
...
server.post('/get-blog', (req, res) => {
    ...
    Blog.findOneAndUpdate({ blog_id }, { $inc: { "activity.total_reads": incrementVal } })
        ...
        .then(blog => {
            ...
            if (blog.draft && !draft) {
                return res.status(500).json({ error: "You can't access Drafts" })
            }
