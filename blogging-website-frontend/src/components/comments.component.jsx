...
const CommentsContainer = () => {
  ...
  return (
    <div id='commScroll' className={'max-sm:w-full fixed ' + (commentsWrapper ? 'top-0 sm:right-0 ' : 'top-[100%] right-[-100%] ') + ' duration-700 ease-out  max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full bg-white z-50 shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden'}>

      <div className='relative'>

        <h1 className='text-xl font-medium'>Comments</h1>
        <p className='text-lg mt-1 w-[70%] text-dark-grey line-clamp-1'>{title}</p>

        <button className='absolute top-0 right-0 flex justify-center items-center w-12 h-12 rounded-full bg-grey hover:bg-black/70 hover:text-white duration-500'
